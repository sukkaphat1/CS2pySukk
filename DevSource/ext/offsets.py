...
class Offset:
	...
	# Optional fields used by the skin-sharing diagnostics. They default to 0
	# so older offset dumps still load; the diagnostic simply stays inactive
	# until a dump provides them.
	m_steamID: int = 0
	dwNetworkGameClient: int = 0
	dwNetworkGameClient_signOnState: int = 0
	dwNetworkGameClient_serverTickCount: int = 0
	dwNetworkGameClient_maxClients: int = 0
...
class Client:
	...
	def optional_field(self, class_name, field_name, default=0):
		try:
			return self.clientdll["client.dll"]['classes'][class_name]['fields'][field_name]
		except (KeyError, TypeError):
			return default

	def optional_root_offset(self, module_name, field_name, default=0):
		try:
			return self.offsets[module_name][field_name]
		except (KeyError, TypeError):
			return default
...
def get_offsets() -> Offset:
	...
	offsets_obj = Offset(
		...
		m_steamID=oc.optional_field("CBasePlayerController", "m_steamID"),
		dwNetworkGameClient=oc.optional_root_offset("engine2.dll", "dwNetworkGameClient"),
		dwNetworkGameClient_signOnState=oc.optional_root_offset("engine2.dll", "dwNetworkGameClient_signOnState"),
		dwNetworkGameClient_serverTickCount=oc.optional_root_offset("engine2.dll", "dwNetworkGameClient_serverTickCount"),
		dwNetworkGameClient_maxClients=oc.optional_root_offset("engine2.dll", "dwNetworkGameClient_maxClients"),
